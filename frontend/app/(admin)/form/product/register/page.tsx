...
export default function Page() {
    ...
    return (<div>
      <div className="flex flex-col items-center gap-5 p-5">
        ...
        <div className="flex gap-5 items-center">
          <div className="flex w-[400px]">
            <Select isRequired label="Type" classNames={{ base: "bg-custom-color1 rounded-lg", trigger: "bg-custom-color5" }} value={selectedType} onChange={(e) => {
        ...
        <div className="flex gap-5 items-center">
          <div className="flex w-[400px]">
            <Select isRequired label="Category" classNames={{ base: "bg-custom-color1 rounded-lg", trigger: "bg-custom-color5" }} value={selectedCategory} onChange={(e) => {
      ...
      <div className="flex flex-col items-center gap-5 m-[15px]">
        ...
        {productDetails.map((detail, index) => (<div key={index} className="flex justify-around font-medium text-lg pb-5 items-start w-full">
            <div className="w-[150px] ">
              {capitaliseWord(colours.find((colour) => colour.colourid.toString() == detail.colourid)?.colourname || "")}
            </div>
            <div className="w-[150px] ">{capitaliseWord(sizes.find((size) => size.sizeid.toString() == detail.sizeid)?.sizename || "")}</div>
        ...
        <div className="flex justify-around font-medium	text-lg	pb-5 items-start	w-full">
          ...
          <div className="w-[150px] ">
            <Input type="number" size={"lg"} label="Quantity" min="0" classNames={{ input: "text-sm", inputWrapper: "border-custom-color1 h-6 rounded-lg" }} className="max-w-[600px]" variant="bordered" value={quantity} onValueChange={setQuantity}/>
          </div>
          <div className="w-[150px] ">
        ...
        <div className="flex justify-around font-medium text-lg pb-5 items-start w-full">
          <div className="w-[150px]">
            <Button variant="ghost" size="md" onClick={saveProduct} className="bg-custom-color1 ms-6 text-white">
              SAVE PRODUCT
            </Button>
          </div>
        </div>
