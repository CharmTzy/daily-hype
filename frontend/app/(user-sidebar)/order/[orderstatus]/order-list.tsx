    ...
    return (<>
      {orderData.map((order, index) => {
            ...
            <div className="flex justify-start py-4 items-center border-1 rounded-tl-lg rounded-tr-lg px-6">
              ...
              {theme === "dark" ? (<Tooltip offset={12} content={<div className="px-2 py-2 ms-2">{orderStatusInfoText[order.orderstatus as keyof OrderStatusInfoText]}</div>}>
                  <div className="w-[15px] h-[15px] relative cursor-pointer ms-2">
                    <Image src="/icons/info-dark.svg" fill={true} alt="Info Icon"/>
                  </div>
                </Tooltip>) : (<Tooltip offset={12} content={<div className="px-2 py-2 ms-2 max-w-lg text-small">{orderStatusInfoText[order.orderstatus as keyof OrderStatusInfoText]}</div>}>
