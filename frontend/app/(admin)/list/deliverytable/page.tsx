...
export default function Page() {
    ...
    async function fetchDataFromPostalCode(postalCode: string) {
        try {
            const nominatimResponse = await axios.get(`https://nominatim.openstreetmap.org/search?postalcode=${postalCode}&country=Singapore&format=json`);
            const { lat, lon } = nominatimResponse.data[0];
            const weatherResponse = await axios.get(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${process.env.NEXT_PUBLIC_APP_WEATHER_URL}`);
            return {
    ...
    const [selectedDeliveriesTransfer, setSelectedDeliveriesTransfer] = useState<number[]>([]);
    useEffect(() => {
        console.log("Selected Deliveries have changed:", selectedDeliveriesTransfer);
    ...
    const handleSelectAllDeliveriesTransfer = () => {
        const allSelected = selectedDeliveriesTransfer.length === deliveryData.length;
        if (allSelected) {
            setSelectedDeliveriesTransfer([]);
        }
        else {
            const allDeliveryIds = deliveryData.map((delivery) => delivery.deliveryId);
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                const response = await fetch(`${process.env.BACKEND_URL}/api/categoriesForDelivery`);
    ...
    useEffect(() => {
        const fetchShippers = async () => {
            try {
                const response = await fetch(`${process.env.BACKEND_URL}/api/shippersForDelivery`);
                const data = await response.json();
                setShippers(data);
            }
            catch (error) {
    ...
    useEffect(() => {
        const fetchUserRegions = async () => {
            try {
                const response = await fetch(`${process.env.BACKEND_URL}/api/userRegionsForDelivery`);
                const data = await response.json();
                const regionNames = data.map((regionObj: {
                    region: string;
                }) => regionObj.region);
    ...
    useEffect(() => {
        const fetchUserNames = async () => {
            try {
                const response = await fetch(`${process.env.BACKEND_URL}/api/userNameForDelivery`);
                const data = await response.json();
                const usernameList = data.map((userObj: {
                    username: string;
                }) => userObj.username);
    ...
    useEffect(() => {
        const fetchproduct = async () => {
            try {
                const response = await fetch(`${process.env.BACKEND_URL}/api/productForDelivery`);
                const data = await response.json();
                const productList = data.map((prodObj: {
                    productname: string;
                }) => prodObj.productname);
    ...
    useEffect(() => {
        const fetchaddress = async () => {
            try {
                const response = await fetch(`${process.env.BACKEND_URL}/api/addressForDelivery`);
                const data = await response.json();
                const addressList = data.map((addObj: {
                    addressname: string;
                }) => addObj.addressname);
    ...
    const renderDeliveryTable = () => {
        const filteredData = selectedDateFilters.length > 0
            ...
            : deliveryData;
        return (<table className="table w-full border border-neon-blue">
        <thead>
          <tr className="bg-neon-blue">
            
            <th className="p-2">
              <input type="checkbox" checked={selectedDeliveriesTransfer.length === filteredData.length} onChange={() => handleSelectAllDeliveriesTransfer()}/>
            </th>
        ...
        <tbody>
          {filteredData.map((delivery) => (<tr key={delivery.deliveryId} className="border-t border-neon-blue">
              
              <td className="p-2">
                
                <input type="checkbox" checked={selectedDeliveriesTransfer.includes(delivery.deliveryId)} onChange={() => handleSelectDeliveryTransfer(delivery.deliveryId)}/>
              </td>
              ...
              <td className="p-2">
                {delivery.unreadMessageCount > 0 ? (<span className="text-neon-green">
    ...
    const handleDeleteReal = async (SelectedDeliveryID2: any) => {
        try {
            console.log("Edited Data:", editFormData);
            const deletionSuccessful = await handleDeleteDeliverySequentially(SelectedDeliveryID2);
        ...
        try {
            ...
            if (overviewOrNot) {
                if ("geolocation" in navigator) {
                    ...
                    console.log("My current location is at " + latitude + ", " + longitude);
                }
                else {
                    throw new Error("Geolocation is not supported by this browser.");
                }
                router.push(`/list/deliverytable/viewlocation?latitude=${latitude}&longitude=${longitude}`);
            }
            else {
                router.push(`/list/deliverytable/viewlocation?latitude=${selectedLoc.latitude}&longitude=${selectedLoc.longitude}`);
            }
        }
        catch (error) {
          ...
          {() => (<>
              ...
              <ModalBody className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={editFormData.orderId} readOnly className={`w-full ${editFormData.orderId ? "readonly" : ""}`}/>
                </div>
                <div>
                  <label>Delivery ID:</label>
                  <input type="text" value={editFormData.deliveryId} readOnly className={`w-full ${editFormData.deliveryId ? "readonly" : ""}`}/>
                </div>
                <div>
                  <label>Tracking Number:</label>
                  <input type="text" value={editFormData.trackingNumber} readOnly className={`w-full ${editFormData.trackingNumber ? "readonly" : ""}`}/>
                </div>
                <div>
                  <label>Delivery Address:</label>
                  <textarea value={editFormData.deliveryAddress} onChange={(e) => setEditFormData({
                ...editFormData,
                deliveryAddress: e.target.value,
            })} className={`w-full ${editFormData.deliveryAddress ? "readonly" : ""}`}/>
                </div>
                <div>
                  <label>Delivery Status Detail:</label>
                  <select value={editFormData.deliveryStatusDetail} onChange={(e) => setEditFormData({
                ...editFormData,
                ...
                <div>
                  ...
                  <input type="datetime-local" id="deliveryDateTime" className={`form-control w-full ${editFormData.deliveryTime ? "readonly" : ""}`} value={editFormData.deliveryTime
                ...
                <div>
                  ...
                  <select value={editFormData.shipping.carrier} onChange={(e) => setEditFormData({
                ...editFormData,
