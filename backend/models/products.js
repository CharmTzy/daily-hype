...
module.exports.getProductsByCategoryIDs = function getProductsByCategoryIDs(categoryIDs, limit, offset, isinstock, typeIDs = []) {
  ...
  `;
    if (typeIDs.length > 0) {
        params.push(typeIDs);
        sql += `
      AND P.typeid = ANY($${params.length}::int[])
    `;
    }
...
module.exports.getProductCountByCategoryIDs = function getProductCountByCategoryIDs(categoryIDs, isinstock, typeIDs = []) {
    const params = [categoryIDs];
    let sql = `
...
module.exports.getProductByLimit = async (limit) => {
    const sql = `
        ...
        ORDER BY p.createdat DESC, p.soldqty DESC, p.productid DESC
        LIMIT $1
      `;
...
module.exports.getBestSellingProductByLimit = async (limit) => {
    const sql = `
        ...
        WHERE p.typeid = t.typeid
        AND p.categoryid = c.categoryid
        ORDER BY p.soldqty DESC, p.rating DESC, p.createdat DESC, p.productid DESC
        LIMIT $1
      `;
