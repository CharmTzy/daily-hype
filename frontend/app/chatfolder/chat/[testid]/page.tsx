...
import { Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, Button, useDisclosure, Progress, Link, } from "@nextui-org/react";
import { getAllMessages, getCurrentUserId, updatechatleavestatus, getDeliveryDetail, } from "@/functions/deliv-functions";
import "./Chatroom.css";
...
const ChatRoomPage = () => {
    ...
    if (actualCurrentUserRole == "admin") {
        backLink2 = (<Link className="text-black" href={URL.DeliveryList}>
        🔙 Back to Admin
      </Link>);
    }
    else if (actualCurrentUserRole == "customer") {
        backLink2 = (<Link className="text-black" href={URL.Delivery}>
        🔙 Back to User
      </Link>);
    }
    else {
    }
    return (<div className="chatroom-container" style={{
            maxWidth: "600px",
            margin: "auto",
            marginTop: "50px",
            overflowY: "auto",
          ...
          <ModalContent>
            ...
            <ModalBody>
              {selectedDelivery && (<>
                  ...
                  <p className="card-text">
                    Time:{" "}
                    {new Date(selectedDelivery[0].deliveryTime).toLocaleTimeString()}
                  </p>
                  <p className="card-text">
                    Status: {selectedDelivery[0].deliveryStatusDetail}
                  </p>
                  <Progress label={`${getProgressValue(selectedDelivery[0].deliveryStatusDetail)}% ${"Progress"}`} value={getProgressValue(selectedDelivery[0].deliveryStatusDetail)} className="max-w-md"/>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {selectedDelivery[0].items.map((item: any, index: number) => (<div key={index} className="text-center">
      ...
      <div className="chatroom-messages">
        {messages.map((message, index) => (<div key={index} className={`message`} style={{
                backgroundColor: actualCurrentUserId == message.speakerid
                    ? "lightgreen"
                    : "lightblue",
            }}>
            <span className="message-text">{message.text}</span>
            {message.messagedatetime && (<span className="message-timestamp relative bottom-0 left-2 text-xs text-gray-500">
