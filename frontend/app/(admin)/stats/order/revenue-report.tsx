...
export default function RevenueReport() {
    ...
    const [pieChartData, setPieChartData] = useState([
        { data: [0, 0, 0, 0], label: "Male", backgroundColor: ["rgba(255, 99, 132, 0.2)", "rgba(54, 162, 235, 0.2)", "rgba(255, 206, 86, 0.2)", "rgba(75, 192, 192, 0.2)"], borderColor: ["rgba(255, 99, 132, 1)", "rgba(54, 162, 235, 1)", "rgba(255, 206, 86, 1)", "rgba(75, 192, 192, 1)"], borderWidth: 1 },
        { data: [0, 0, 0, 0], label: "Female", backgroundColor: ["rgba(255, 99, 132, 0.2)", "rgba(54, 162, 235, 0.2)", "rgba(255, 206, 86, 0.2)", "rgba(75, 192, 192, 0.2)"], borderColor: ["rgba(255, 99, 132, 1)", "rgba(54, 162, 235, 1)", "rgba(255, 206, 86, 1)", "rgba(75, 192, 192, 1)"], borderWidth: 1 },
    ]);
    const generateTable = (quarter: string, gender: string, year: number) => {
        let tempQuarter = 4;
        ...
        getAdminOrderQuarterRevenueDetail(tempQuarter, tempGender, year).then((result) => {
            if (result.error) {
            }
            else {
                ...
                setTableData2(product.map((item, index) => {
                    return [
    ...
    useEffect(() => {
        const timeout = setTimeout(() => {
            getAdminOrderQuarterRevenueStat(year).then((result) => {
                if (result.error) {
                    ...
                    tempData.forEach((item) => {
                        if (item.gender === "M") {
                            tempData2[0].data[parseInt(item.quarter) - 1] = parseFloat(item.revenue);
                        }
                        else {
                            tempData2[1].data[parseInt(item.quarter) - 1] = parseFloat(item.revenue);
                        }
                    });
                    setData(tempData2);
                }
    ...
    return (<div>
      <div className="flex justify-center mt-8">
        <div className="flex items-center mr-8">
          ...
          <Select indicator={<KeyboardArrowDown />} size="sm" value={year} onChange={(e, newValue) => {
            ...
            {yearOptions.map((item, index) => (<Option value={item} key={index}>
                {item}
              </Option>))}
          </Select>
        </div>
        <div className="flex items-center ms-8">
          ...
          <Select indicator={<KeyboardArrowDown />} size="sm" value={chart} onChange={(e, newValue) => {
            if (newValue === "bar" || newValue === "line" || newValue === "pie")
                setChart(newValue);
        }} sx={{
            width: "250px",
            [`& .${selectClasses.indicator}`]: {
      ...
      {chart === "bar" && <BarChart className="mt-2 mb-4" chartTitle="Revenue by Quarter" labels={["Quarter 1", "Quarter 2", "Quarter 3", "Quarter 4"]} datasets={barChartData} onClick={(x, y, label) => generateTable(x, label, year)}/>}
      {chart === "line" && <LineChart className="mt-2 mb-4" chartTitle="Revenue by Quarter" labels={["Quarter 1", "Quarter 2", "Quarter 3", "Quarter 4"]} datasets={lineChartData} onClick={(x, y, label) => generateTable(x, label, year)}/>}
      {chart === "pie" && <PieChart chartTitle="Revenue by Quarter" labels={["Quarter 1", "Quarter 2", "Quarter 3", "Quarter 4"]} datasets={pieChartData} onClick={(value) => alert(value)} className="mb-4"/>}
