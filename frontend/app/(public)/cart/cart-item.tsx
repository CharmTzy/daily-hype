...
export default function CartItem({ data, cart, setCart, index, disabledCheckBox, setDisableCheckBox, isAuthenticated, setReload, setDeleteIndex }: ICartItemProps) {
    ...
    return (<div className="flex w-full max-w-full items-center mt-4">
      ...
      <CartItemQty qty={cart.qty} handleQtyChange={(newQty) => {
            if (newQty <= 0) {
        ...
        }} handleDelete={() => setDeleteIndex(index)} disabled={disabled} price={data.unitprice}/>
    </div>);
}
