...
export default function ProductList({ productData, setLimit, pageNo, setPageNo, productCount }: IProductListProps) {
    ...
    return (<div className="w-full max-w-full px-4 py-2">
      <div className="py-4">
        <label className="text-large font-semibold">Product List</label>
      </div>
      <div className="mb-5">
        <CustomTable columns={columns} rows={productData} onClick={(clickedValue) => {
            alert(clickedValue);
        }} setRowsPerPage={setLimit} page={pageNo} setPage={setPageNo} totalCount={productCount}/>
      </div>
    </div>);
}
