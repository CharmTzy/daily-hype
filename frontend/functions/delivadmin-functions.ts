...
export async function getAllDeliveriesAdmin(userId: any, filterOptions: any) {
    try {
        const { startDate, endDate, startOrderDate, endOrderDate, statusDetail, createdAtSortOrder, deliveryDateSortOrder, chatunread, username, product, userRegion, searchCategory, address, searchShipper, limit, offset, } = filterOptions;
        ...
        const response = await fetch(`${process.env.BACKEND_URL}/api/AlldeliveriesPageAdmin/user/${userId}?${queryString}`, {
            method: "GET",
            credentials: "include",
        });
        if (!response.ok) {
...
export async function handleDeleteDeliverySequentially(deliveryId: any) {
    try {
        ...
        const removeDeliveryResponse = await fetch(`${process.env.BACKEND_URL}/api/removeDeliveryIDFromOrder/${deliveryId}`, { method: "POST", credentials: "include" });
        ...
        const deletionResponse = await fetch(`${process.env.BACKEND_URL}/api/deliveriesCA2/${deliveryId}`, { method: "DELETE", credentials: "include" });
...
export async function updateSingleDeliveryCA2(editFormData: any) {
    try {
        ...
        const response = await fetch(`${process.env.BACKEND_URL}/api/updateDeliveryCA2`, {
            method: "PUT",
            credentials: "include",
            headers: {
...
export async function retrieveArrayChartJS(selectedDropdownValueForm: any, choiceNum: any, date1False: any, date2False: any) {
    ...
    try {
        ...
        const deliveryIds = data.map((item: {
            deliveryid: any;
        }) => item.deliveryid);
        const chartHeader = data.map((item: {
            chartHeaderValue: any;
        }) => item.chartHeaderValue);
        ...
        const [getChartPart2URLResponseSpecific, getChartPart2URLResponse] = await Promise.all([
        ...
        const [additionalDataPart2and3Ids, additionalDataPart2and3] = await Promise.all([
        ...
        const groupedDeliveries = data0.reduce((acc: any, obj: any) => {
            ...
            const existingGroup = acc.find((group: any) => group.currentstagenum === stageNum);
            if (existingGroup) {
        ...
        groupedDeliveries.sort((a: any, b: any) => a.currentstageNum - b.currentstageNum);
        console.log(groupedDeliveries);
        const additionalDataPart2ids = additionalDataPart2and3Ids.stageintervalSpecific;
        ...
        const additionalDataPart2idsArr = Object.entries(additionalDataPart2ids).map(([category, data]) => ({
            category,
            deliveries: (data as CategoryData).deliveries.map((delivery: Delivery) => ({
        ...
        const additionalDataPart3idsArr = Object.entries(additionalDataPart3ids).map(([category, data]) => ({
            category,
            deliveries: (data as CategoryData).deliveries.map((delivery: Delivery) => ({
                deliveryid: delivery.deliveryid,
                rows: delivery.rows.map((row: Row) => ({
