...
export async function getAdminReview(pageNo: number, limit: number): Promise<IAdminReviewFetch> {
    return fetch(`${process.env.BACKEND_URL}/api/reviews/admin?offset=${pageNo * limit}&limit=${limit}`, {
        method: "GET",
        credentials: "include",
    })
...
export function getAdminReviewStat(year: number): Promise<IAdminReviewStat> {
    return fetch(`${process.env.BACKEND_URL}/api/review/stat/revenue/quarter?year=${year}`, {
        method: "GET",
        credentials: "include",
    })
        .then((response) => response.json())
