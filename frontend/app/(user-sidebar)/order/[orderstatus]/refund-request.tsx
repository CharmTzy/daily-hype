...
export default function RefundRequest({ isOpen, onClose, orderID }: IRefundRequestProps) {
    ...
    return (<Modal size="5xl" isOpen={isOpen} onClose={onClose} hideCloseButton={true} isDismissable={false} scrollBehavior="inside">
      <ModalContent>
        {(onClose) => (<>
            ...
            <ModalFooter>
              ...
              <Button className="disabled:cursor-not-allowed" color="primary" disabled={textCount > 500 || textCount === 0 || uploadedImages.length <= 0 || productData.length <= 0 || isLoading} onClick={() => {
