...
export default function OrderFilter({ searchMonth, setSearchMonth, searchYear, setSearchYear, showOrderNo, setShowOrderNo, searchOrder, setSearchOrder }: orderFilterProps) {
    ...
    return (<div className="flex w-full justify-between mb-5 items-center">
      <div>
        <Input isClearable type="text" placeholder="Search Order" variant="bordered" size="sm" radius="sm" startContent={<SearchIcon width={15} height={15} className="flex items-center justify-center"/>} value={searchOrder} className="w-[350px] ms-auto" classNames={{ inputWrapper: "h-10", input: "text-[12px]" }} onChange={(e) => {
