...
export default function Page() {
    ...
    const formatOrderList = (data: IAdminOrder[]) => {
        return data.map((item, index) => {
          ...
          {item.orderstatus === "cancelled" && (<Chip size="sm" color="danger">
              {capitaliseWord(item.orderstatus)}
            </Chip>)}
          {item.orderstatus === "in progress" && (<Chip size="sm" color="default">
              {capitaliseWord(item.orderstatus)}
            </Chip>)}
          {item.orderstatus === "received" && (<Chip size="sm" color="success">
              {capitaliseWord(item.orderstatus)}
            </Chip>)}
          {item.orderstatus === "delivered" && (<Chip size="sm" color="warning">
              {capitaliseWord(item.orderstatus)}
            </Chip>)}
          {item.orderstatus === "confirmed" && (<Chip size="sm" color="primary">
              {capitaliseWord(item.orderstatus)}
            </Chip>)}
        </label>,
                <div className="flex justify-center" key={index}>
          {item.orderstatus === "in progress" && (<div className="flex flex-col justify-center items-center">
              <Button className="mb-2" variant="bordered" size="sm" onClick={() => router.push(URL.AdminOrderDetail + item.orderid)}>
                View
              </Button>
              <Button color="primary" className="mb-2" size="sm" onClick={() => {
          ...
          {["cancelled", "confirmed", "delivered", "received"].includes(item.orderstatus) && (<Button variant="bordered" size="sm" onClick={() => router.push(URL.AdminOrderDetail + item.orderid)}>
              View
            </Button>)}
        </div>,
            ] as [
