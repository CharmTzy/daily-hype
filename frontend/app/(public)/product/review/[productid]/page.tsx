...
export default function ProductReviews() {
    ...
    const renderStarIcons = (rating: number) => {
        ...
        for (let i = 1; i <= 5; i++) {
            if (i <= roundedRating) {
                stars.push(<StarIcon key={i} className={`text-yellow-400 w-4 h-4`}/>);
            }
            else if (i === roundedRating + 1 && rating % 1 !== 0) {
                stars.push(<StarHalfIcon key={i} className={`text-yellow-400 w-4 h-4`}/>);
            }
            else {
                stars.push(<StarIcon key={i} className={`text-gray-300 w-4 h-4`}/>);
            }
        }
        return stars;
    };
      ...
      <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-6">
          <div className="rating-filter">
            <select className="border border-black" onChange={(e) => handleRatingFilter(parseInt(e.target.value))}>
             <option value={String(null)}>All Ratings</option>
              {[1, 2, 3, 4, 5].map((rating) => (<option key={rating} value={String(rating)}>
                  {rating} Stars
                </option>))}
            </select>
          ...
          {filteredReviews.map((review, index) => (<div key={index} className="mt-6">
              <div className="mt-4 border-b border-gray-200">
                <div className="flex justify-between">
                  ...
                  <div className="flex space-x-4 ml-20">
                    {review.urls.map((url: string, i: number) => (<img key={i} src={url} alt={`Review Image ${i + 1}`} className="w-20 h-20"/>))}
                  </div>
