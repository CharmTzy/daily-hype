...
router.get("/productsByCategoryIDs", function (req, res) {
    ...
    const typeIDs = req.query.typeIDs ? req.query.typeIDs.split(",").filter((id) => !isNaN(id)).map((id) => parseInt(id, 10)) : [];
    ...
    return productsModel
        .getProductsByCategoryIDs(categoryIDs, limit, offset, isinstock, typeIDs)
        .then((product) => {
        const getImageExecute = [];
        const getProductColourExecute = [];
        product.forEach((item) => {
...
router.get("/productsCountByCategoryIDs", function (req, res) {
    ...
    return productsModel
        .getProductCountByCategoryIDs(categoryIDs, isinstock, typeIDs)
        .then((productCount) => {
        return res.json({ productCount: productCount[0].productcount });
    })
        .catch((error) => {
...
router.get("/product/latest", (req, res) => {
    ...
    return redis
        .getOrSetCache(`latest:v2?limit=${limit}`, () => {
...
router.get("/product/bestselling", (req, res) => {
    ...
    return redis
        .getOrSetCache(`bestselling:v2?limit=${limit}`, () => {
