...
export default function Page({ params }: ProductFormProps) {
    ...
    return (<div>
      <div className="flex flex-col items-center gap-5 p-5">
        ...
        <div className="flex">
          {images.map((image, index) => (<img key={index} src={image} alt={`Uploaded Image ${index}`} style={{ width: '100px', height: '100px', marginRight: '10px' }}/>))}
      ...
      <div className="flex flex-col items-center gap-5  m-[15px] border-2">
        ...
        {productDetails.map((detail, index) => (<div key={index} className="flex justify-around font-medium text-lg pb-5 items-start w-full">
            <div className="w-[150px] ">
              ...
              <div key={index} title={capitaliseWord(detail.colour)} className={`w-4 h-4 border-1 mr-1 rounded-full laptop-3xl:w-5 laptop-3xl:h-5`} style={{ backgroundColor: `#${detail.hex}` }}></div>

            </div>

            <div className="w-[150px] ">{capitaliseWord(detail.size)}</div>
      ...
      {dataToModal && <DetailUpdate isOpen={isOpen} onClose={onClose} productDetails={dataToModal} onModalClose={() => {
                setButtonClicked(!buttonClicked);
            }}/>}
    </div>);
}
