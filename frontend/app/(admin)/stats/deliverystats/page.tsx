...
export default function Page() {
    ...
    const handleFilterClick = async () => {
        ...
        switch (selectedFilterOption) {
            case "category":
                ...
                retrieveChart(selectedCategory, 2, formatDateToString(startDate), formatDateToString(endDate));
                break;
            case "region":
                console.log("Region filter selected");
                retrieveChart(selectedRegion, 3, formatDateToString(startDate), formatDateToString(endDate));
                break;
            case "general":
                console.log("General filter selected");
                if (selectedGeneral === "DayParts") {
                    console.log("Going to 4th option");
                    retrieveChart(selectedGeneral, 4, formatDateToString(startDate), formatDateToString(endDate));
                }
                else {
                    console.log("Going to 1st option");
                    retrieveChart(selectedGeneral, 1, formatDateToString(startDate), formatDateToString(endDate));
                }
                break;
            default:
        ...
        async function retrieveChart(vartarget: any, choiceNumPara: any, startDate: any, endDate: any) {
            ...
            const date2 = endDate;
            const result = await retrieveArrayChartJS(selectedDropdownValueForm, choiceNum, date1, date2);
    ...
    useEffect(() => {
        ...
        const processedChartData0 = {
            labels: resultData[4]?.map((item: any) => item.currentstagenum) || [],
            datasets: [
                {
    ...
    const chartOptions = {
        ...
        onClick: (event: any, chartElements: any) => {
            if (chartElements && chartElements.length > 0) {
                ...
                var checkwhichchart = chartElements[0].element.$context.dataset.additionalNote;
                ...
                if (checkwhichchart == "Delivery Stage Interval") {
                    ...
                    var clickedCategoryDeliveries = resultData[2].find((category) => category.category === clickedCategory)?.deliveries;
                    setChart1OpenClose(false);
                }
                else if (checkwhichchart == "Late Delivery Hours") {
                    const clickedCategory = chartDataStore2.labels[index];
                    console.log("clickedCategory is " + clickedCategory);
                    var clickedCategoryDeliveries = resultData[3].find((category) => category.category === clickedCategory)?.deliveries;
                    setChart2OpenClose(false);
                }
                else if (checkwhichchart == "Number At Each Stage") {
                    const clickedCategory = chartDataStore0.labels[index];
                    var clickedCategoryDeliveries = resultData[4].find((currentstagenum) => currentstagenum.currentstagenum === clickedCategory)?.deliveries;
    ...
    return (<div>
      ...
      <div className="p-4 space-y-4 max-w-md mx-auto">
        ...
        <div>
          <label htmlFor="allDropdown" className="block text-sm font-medium text-gray-700">
            Select Period:
          </label>
          <select id="allDropdown" disabled={selectedFilterOption !== "general"} className="mt-1 p-2 border border-gray-300 rounded-md w-full" onChange={(e) => setSelectedGeneral(e.target.value)}>
        ...
        <div>
          <label htmlFor="categoryDropdown" className="block text-sm font-medium text-gray-700">
            Select Category:
          </label>
          <select id="categoryDropdown" className="mt-1 p-2 border border-gray-300 rounded-md w-full" disabled={selectedFilterOption !== "category"} value={selectedFilterOption === "category" ? selectedCategory : ""} onChange={(e) => setSelectedCategory(e.target.value)}>
        ...
        <div>
          <label htmlFor="regionDropdown" className="block text-sm font-medium text-gray-700">
            Select User Region:
          </label>
          <select id="regionDropdown" className="mt-1 p-2 border border-gray-300 rounded-md w-full" disabled={selectedFilterOption !== "region"} value={selectedFilterOption === "region" ? selectedRegion : ""} onChange={(e) => setSelectedRegion(e.target.value)}>
            <option value="RegPlaceholder">Select a Region</option>

            {userRegions.map((region) => (<option key={region.region} value={region.region}>
                {region.region}
              </option>))}
          </select>
        </div>
        ...
        <div className="text-center">
          <div>
            ...
            <DatePicker selected={startDate} onChange={(date) => setStartDate(date)} dateFormat="yyyy/MM/dd" isClearable placeholderText="  Select Start Date"/>
          </div>

          <div className="mt-2">
            <label>End Date:</label>
            <DatePicker selected={endDate} onChange={(date) => setEndDate(date)} dateFormat="yyyy/MM/dd" isClearable placeholderText="  Select End Date"/>
          </div>
        </div>
        ...
        <div className="flex items-center mt-5 mx-5">
          <input type="radio" id="generalFilter" name="filterOption" value="general" checked={selectedFilterOption === "general"} onChange={() => setSelectedFilterOption("general")}/>
          <label htmlFor="generalFilter" className="ml-2">
            General
          </label>

          <input type="radio" id="categoryFilter" name="filterOption" value="category" className="ml-3" checked={selectedFilterOption === "category"} onChange={() => setSelectedFilterOption("category")}/>
          <label htmlFor="categoryFilter" className="ml-2">
            Category
          </label>

          <input type="radio" id="regionFilter" name="filterOption" value="region" checked={selectedFilterOption === "region"} onChange={() => setSelectedFilterOption("region")} className="ml-4"/>
      ...
      <div className="text-center">
        ...
        <button className="bg-green-500 ml-4 text-white px-4 py-2 rounded-md hover:bg-yellow-600 focus:outline-none focus:ring focus:border-blue-700 my-4" onClick={() => {
            setIsTableOpen(!isTableOpen);
            if (!isTableOpen) {
                setSelectedDataSpecific([]);
            }
        }}>
          {isTableOpen ? "Close All Table" : "Allow Tables"}
        </button>

        <button className="bg-blue-500 ml-4 text-white px-4 py-2 rounded-md hover:bg-blue-600 focus:outline-none focus:ring focus:border-blue-700 my-4" onClick={handleFilterClick}>
          Filter
        </button>
        ...
        {selectedDataSpecific.length > 0 &&
            ...
            !Chart0OpenClose &&
            Chart1OpenClose &&
            Chart2OpenClose && (<div className="flex justify-center">
              <table className="border-collapse border border-black">
                ...
                <tbody>
                  {selectedDataSpecific.map((delivery: any) => (<tr key={delivery.deliveryid}>
        ...
        {selectedDataSpecific.length > 0 &&
            ...
            Chart2OpenClose && (<div className="flex justify-center">
              <table className="border-collapse border border-black">
                ...
                <tbody>
                  {selectedDataSpecific.map((delivery: any) => delivery.rows.map((row: any) => {
                if ("diff_ab_hours" in row &&
                    "diff_bc_hours" in row &&
                    "diff_cd_hours" in row) {
                    return (<tr key={row.deliveryid}>
                            <td className="border border-black p-2">
                              {row.deliveryid}
                            </td>
                            <td className="border border-black p-2">
