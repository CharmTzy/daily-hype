...
import { getAllOrderItems, createDelivery, updateDeliveryForOrderItem, updateOrderStatusAndCreateChatRoom, } from "@/functions/deliv-functions";
...
export default function Page() {
    ...
    const [defaultFirstCreatedDeliveryID, setdefaultFirstCreatedDeliveryID] = useState("");
    const [deliveryIDs, setDeliveryIDs] = useState<number[]>([]);
    const [latestDelivery, setLatestDelivery] = useState<any | null>(null);
    const [selectedDeliveryIDs, setSelectedDeliveryIDs] = useState<number[]>(Array(orderItems.length).fill(""));
    const handleDeliveryIDSelection = (index: number, deliveryID: number) => {
        console.log("delivery index is " + index + " and deliveryID is " + deliveryID);
    ...
    const handleCreateDelivery = async () => {
        try {
            ...
            const newDeliveryId = await createDelivery(orderID, deliveryDate, "UnDelivered", deliveryStatusDetail, trackingNumber, shipid);
    ...
    const handleSendToBackend = async () => {
        try {
            ...
            for (let index = 0; index < orderItems.length; index++) {
                ...
                var deliveryID = FormalselectedDeliveryIDs[index];
                if (deliveryID) {
                }
                else {
                ...
                const updatedProductDetailID = await updateDeliveryForOrderItem(deliveryID, orderID, productDetailID);
                console.log(`Product Detail ID updated for order item ${index}: ${updatedProductDetailID}`);
                const updatedToConfirmedOrderID = updateOrderStatusAndCreateChatRoom(deliveryID, orderID);
    ...
    return (<div>
      ...
      {latestDelivery && latestDelivery.length > 0 && (<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {latestDelivery.map((delivery: any, index: any) => (<Card key={index} className="mb-4">
              <CardBody>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        <ModalContent>
          ...
          <ModalBody>
            ...
            <input type="datetime-local" id="deliveryDateTime" className="form-control" value={deliveryDate} onChange={(e) => setDeliveryDate(e.target.value)}/>
            
            <select id="deliveryStatusDetail" className="form-control" value={deliveryStatusDetail} onChange={(e) => setDeliveryStatusDetail(e.target.value)}>
            ...
            <input type="text" placeholder="Tracking Number" value={"Tracking No.: " + trackingNumber} readOnly/>

            
            <input type="text" placeholder="Order Address" value={"Delivery Address: " + orderAddress} readOnly/>

            <select id="shipperName" className="form-control" value={shipperName} onChange={(e) => setShipperName(e.target.value)}>
              {shipperItems.map((stage: any, index: any) => (<option key={"shipkey" + index} value={stage.name}>
                  {`Shipper: ${stage.name} `}
                </option>))}
            </select>
          </ModalBody>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {orderItems.map((item, index) => (<Card key={index} className="mb-4">
            <CardBody>
              ...
              <select id={`deliveryDropdown_${index}`} className="form-control" value={selectedDeliveryIDs[index]} onChange={(e) => handleDeliveryIDSelection(index, parseInt(e.target.value, 10))}>
                {deliveryIDs.map((id) => (<option key={id} value={id}>
                    {`Delivery ID: ${id}`}
                  </option>))}
              </select>
            </CardBody>
