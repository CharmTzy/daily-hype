...
module.exports.checkOrderInPaymentTableAsync = async function checkOrderInPaymentTableAsync(orderID) {
...
module.exports.checkIfDeliveryInOrderTable = async function checkIfDeliveryInOrderTable(deliveryIDs) {
    const results = await Promise.all(deliveryIDs.map(async (deliveryID) => {
        const query = `
          SELECT deliveryid
          FROM productorder
          WHERE deliveryid = $1;
...
module.exports.retrieveuserrole = function retrieveuserrole(userid) {
...
module.exports.retrieveDistinctDeliveryIdsWithPagination = function retrieveDistinctDeliveryIdsWithPagination(userid, startDate, endDate, startDateOrder, endDateOrder, statusDetail, limit, offset) {
    let sql = `
